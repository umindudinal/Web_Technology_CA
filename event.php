...
<div class="container">
   ...
   <section class="section_2">
      <div class="card_section">
         ...
         <?php
         $query = "SELECT * FROM all_events ORDER BY created_at DESC";
         $result = mysqli_query($connection, $query);
         ?>
            ...
            <div class="item">
               <div class="image">
                  <?php if (file_exists($imagePath)) { ?>
                     <img src="<?php echo htmlspecialchars($imagePath); ?>" alt="Event Image" width="200px">
                  <?php } else { ?>
               ...
               <div class="content">
                  <div class="top">
                     <div class="date">
                        <p><?php echo htmlspecialchars($month); ?></p>
                  ...
                  <div class="bottom">
                     <div class="location">
                        <i class="fa-solid fa-location-dot"></i> <?php echo htmlspecialchars($row['venue']); ?>
                     </div>

                     <div class="favorit">
                        <?php
                        if (!$authenticated) {
                           echo '<a href="API/login.php" class="btn">Login <i class="fa-solid fa-lock"></i></a>';
                        }
                        elseif ($userRole !== "admin") {
                           $reg_no = $_SESSION["registration_no"];
                           $event_code = $row['event_code'];

                           $stmt = mysqli_prepare($connection, "SELECT 1 FROM registration WHERE registration_no = ? AND event_code = ?");
                           mysqli_stmt_bind_param($stmt, "ss", $reg_no, $event_code);
                           ...
                           if ($already) {
                              echo '<button class="btn registered" disabled>Already Registered <i class="fa-solid fa-check"></i></button>';
                           } else {
                              $safeCode = htmlspecialchars($event_code, ENT_QUOTES);
                              echo '
                              <form class="registerForm" method="post" action="API/event_register.php" onsubmit="return handleRegisterSubmit(this);">
                                    <input type="hidden" name="event_code" value="' . $safeCode . '">
                                    <button type="submit" class="btn addNow">Add Now <i class="fa-regular fa-bookmark"></i></button>
                              </form>
                              ';
                           }
                        }
   ...
   <script>
      ...
      // Optimistic UI: disable and change text immediately
      btn.disabled = true;
      btn.classList.add('registered');
      btn.innerHTML = 'Already Registered <i class="fa-solid fa-check"></i>';

      // Allow the form to submit normally (page will redirect back)
      return true;
      }
   </script>
